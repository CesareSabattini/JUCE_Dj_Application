...
/*
In the constructure the labels and the slider that compose the interface are
initialized. The sliders' ranges, styles, textBoxStyles are set the CommonEffect
class is configured as their listener. The callbacks are declared in the public
scope and defined in the MainComponent. Like in the other user defined
components, the pre-defined ones are styled using the LookAndFeel_V4 inherithed
class AppLAF.
*/
CommonEffects::CommonEffects() {
    ...
    delayTimeLabel.setColour(juce::Label::outlineColourId,
                             juce::Colours::silver);
    ...
    delayWetLabel.setColour(juce::Label::outlineColourId,
                            juce::Colours::silver);
...
void CommonEffects::resized() {
    ...
    reverbLabel.setBounds(borderGap, 0, widthFraction - 2 * borderGap,
                          labelHeight);
    delayLabel.setBounds(widthFraction + borderGap, 0,
                         widthFraction - 2 * borderGap, labelHeight);
    comingSoonLabel.setBounds(2 * widthFraction + borderGap, 0,
                              widthFraction - 2 * borderGap, labelHeight);

    roomSlider.setBounds(borderGap, heightFraction,
                         (widthFraction - 2 * borderGap), heightFraction);
    dampingSlider.setBounds(borderGap, heightFraction * 2,
                            (widthFraction - 2 * borderGap), heightFraction);
    wetLevelSlider.setBounds(borderGap, heightFraction * 3,
                             (widthFraction - 2 * borderGap), heightFraction);
    dryLevelSlider.setBounds(borderGap, heightFraction * 4,
                             (widthFraction - 2 * borderGap), heightFraction);
    widthSlider.setBounds(borderGap, heightFraction * 5,
                          (widthFraction - 2 * borderGap), heightFraction);

    delayTime.setBounds(widthFraction + borderGap, heightFraction,
                        (widthFraction - 2 * borderGap), heightFraction);
    delayFeedback.setBounds(widthFraction + borderGap, heightFraction * 2,
                            (widthFraction - 2 * borderGap), heightFraction);
    delayWet.setBounds(widthFraction + borderGap, heightFraction * 3,
                       (widthFraction - 2 * borderGap), heightFraction);

    const int labelWidth = roomSlider.getBounds().getWidth() / 2;

    roomLabel.setBounds(getBounds().getWidth() / 6 - labelWidth / 2,
                        heightFraction - 10, labelWidth, heightFraction / 3);
    dampingLabel.setBounds(getBounds().getWidth() / 6 - labelWidth / 2,
                           2 * heightFraction - 10, labelWidth,
                           heightFraction / 3);
    wetLabel.setBounds(getBounds().getWidth() / 6 - labelWidth / 2,
                       3 * heightFraction - 10, labelWidth, heightFraction / 3);
    dryLabel.setBounds(getBounds().getWidth() / 6 - labelWidth / 2,
                       4 * heightFraction - 10, labelWidth, heightFraction / 3);
    widthLabel.setBounds(getBounds().getWidth() / 6 - labelWidth / 2,
                         5 * heightFraction - 10, labelWidth,
                         heightFraction / 3);

    delayTimeLabel.setBounds(getBounds().getWidth() / 2 - labelWidth / 2,
                             heightFraction - 10, labelWidth,
                             heightFraction / 3);
    delayFbLabel.setBounds(getBounds().getWidth() / 2 - labelWidth / 2,
                           2 * heightFraction - 10, labelWidth,
                           heightFraction / 3);
    delayWetLabel.setBounds(getBounds().getWidth() / 2 - labelWidth / 2,
                            3 * heightFraction - 10, labelWidth,
                            heightFraction / 3);
}

/*
Definition of the slider cb function, called when a slider mutates. It
distinguishes the origins of the changes using references and modifies the
related parameters.
*/

void CommonEffects::sliderValueChanged(juce::Slider *slider) {
    if (slider == &roomSlider || slider == &dampingSlider ||
        slider == &wetLevelSlider || slider == &dryLevelSlider) {
    ...
    } else if (slider == &delayTime || slider == &delayFeedback ||
               slider == &delayWet) {
