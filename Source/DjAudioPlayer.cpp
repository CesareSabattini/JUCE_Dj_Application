...
void DjAudioPlayer::getNextAudioBlock(
    ...
    /*
    play the audio block through the resample source. The first time the effects
    aren't applied, but this method is called 44100 times per second, so...
    */
    resampleSource.getNextAudioBlock(bufferToFill);
    /*
    Prevent the buffer to get a write pointer to an unexistant channel iterating
    through the available ones. In each channel apply the reverb to the samples
    contained into the bufferToFill. If the number of available channels is 2,
    then the appropriate inner method is called.
    */
    if (bufferToFill.buffer->getNumChannels() != 2) {
        for (int i = 0; i < bufferToFill.buffer->getNumChannels(); i++) {
            reverb.processMono(bufferToFill.buffer->getWritePointer(i),
...
void DjAudioPlayer::pause(void) { transportSource.stop(); }
...
/*
This method applies the delay effect "in place" to a block of samples contained
in a buffer, iterating through its available channels. Getting the write ptrs,
it creates a feedback effect using thejuce::DelayLine::popSample() method. Then
the output is pushed in the buffer and mixed with the original one, blending it
depending on the wet level.
*/

void DjAudioPlayer::applyDelay(juce::AudioBuffer<float> &buffer,
                               int numSamples) {
    for (int channel = 0; channel < buffer.getNumChannels(); ++channel) {
        ...
        for (int i = 0; i < numSamples; ++i) {
            ...
            float out = delayLine.popSample(
                channel,
                static_cast<float>(delayTime * currentSampleRate / 1000.0f),
                true);
...
void DjAudioPlayer::setDelayParameters(float delayTime, float feedback,
                                       float wetLevel) {
...
void DjAudioPlayer::setReverbParameters(
    const juce::Reverb::Parameters &params) {
